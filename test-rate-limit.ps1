...
try {
    ...
    Write-Host "[OK] Gateway is accessible (Status: $($testResponse.StatusCode))" -ForegroundColor Green
    Write-Host ""
}
catch {
    $testStatusCode = $null
    ...
    if ($testStatusCode -eq 404) {
        Write-Host "[ERROR] Gateway returned 404. Possible issues:" -ForegroundColor Red
        ...
        Write-Host ""
        Write-Host "To start the gateway, run: npm run start:dev" -ForegroundColor Cyan
        Write-Host ""
        exit 1
    }
    else {
        Write-Host "[ERROR] Cannot connect to gateway at $baseUrl" -ForegroundColor Red
        Write-Host "  Error: $($_.Exception.Message)" -ForegroundColor Yellow
        Write-Host ""
        Write-Host "Make sure the gateway is running: npm run start:dev" -ForegroundColor Cyan
        Write-Host ""
        exit 1
    }
}
...
elseif ($successCount -eq 0) {
    Write-Host "[ERROR] No successful requests. Check if:" -ForegroundColor Red
    Write-Host "  1. Gateway is running (npm run start:dev)" -ForegroundColor Yellow
...
elseif ($successCount -eq 11) {
    Write-Host "[WARNING] All requests succeeded. Rate limiting may not be active or limit is higher than 10." -ForegroundColor Yellow
    Write-Host "  Check ThrottlerModule configuration in apps/gateway/src/gateway.module.ts" -ForegroundColor Yellow
}
else {
    Write-Host "[WARNING] Rate limiting may not be working (no 429 responses)" -ForegroundColor Yellow
